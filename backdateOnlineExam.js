...
const USERNAME = 'lateefshow21';
const USEREMAIL = 'lateefshow21@gmail.com';
const REPO_NAME = 'OnlineExamination-PHP';      // GitHub repo name
const PROJECT_NAME = 'OnlineExamination-PHP';   // Local folder name
const START_DATE = '2019-09-01';                // Backdate from
const END_DATE = '2020-09-01';                  // Backdate to
...
if (!fs.existsSync(targetDir)) {
    ...
    console.error(`Run from parent folder: node "${PROJECT_NAME}/backdateOnlineExam.js"`);
    process.exit(1);
}
console.log(`Processing: ${targetDir}`);

// === 1. LIST ALL FILES (SKIP RESERVED) ===
let allFiles = [];
    ...
    try {
        ...
        items.forEach(item => {
            ...
            if (stats.isFile()) {
                ...
                if (!['node_modules', '.git', '.vscode', 'vendor', '.git.backup'].includes(base)) {
...
try {
    ...
    if (fs.existsSync(gitDir)) {
        ...
        execSync(`rmdir /s /q "${gitDir}"`, { stdio: 'ignore', shell: 'cmd.exe' });
    ...
    groups.forEach((group, i) => {
        ...
        const msg = `Update: Added ${group.length} files (${sample}) - Online Exam progress`;

        const listFile = `.git-add-list-${i}.txt`;
        fs.writeFileSync(listFile, group.join('\n'), 'utf8');

        const safePath = `"${path.resolve(listFile).replace(/"/g, '\\"')}"`;
